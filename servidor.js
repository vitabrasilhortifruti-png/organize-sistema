...
async function openDB() {
  ...
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS insumos (
      id TEXT PRIMARY KEY, nome TEXT NOT NULL, unidade TEXT DEFAULT 'saco',
      quantidade_atual REAL DEFAULT 0, quantidade_minima REAL DEFAULT 0,
      obs TEXT DEFAULT '', criado TEXT DEFAULT (datetime('now'))
    );
    CREATE TABLE IF NOT EXISTS movimentos_insumo (
      id TEXT PRIMARY KEY, insumo_id TEXT NOT NULL, tipo TEXT NOT NULL,
      quantidade REAL NOT NULL, motivo TEXT DEFAULT '', data TEXT,
  ...
  try { await db.run("ALTER TABLE entradas ADD COLUMN insumo_id TEXT DEFAULT ''"); } catch(e) {}
  try { await db.run("ALTER TABLE descartes ADD COLUMN responsavel TEXT DEFAULT ''"); } catch(e) {}
...
crudRoutes('insumos',        ['nome','unidade','quantidade_atual','quantidade_minima','obs']);
crudRoutes('movimentos_insumo', ['insumo_id','tipo','quantidade','motivo','data']);
...
crudRoutes('descartes',      ['lote_id','lote','fruta','quantidade','motivo','data','responsavel']);
