...
async function enviarBackupEmail() {
  try {
    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const backupFile = path.join(BACKUP_DIR, `backup_semanal_${ts}.db`);
    ...
    await transporter.sendMail({
      ...
      to: 'nildomoraesagro@gmail.com.br',
      subject: `📦 Organize — Backup Semanal ${new Date().toLocaleDateString('pt-BR')}`,
      text: `Olá Nildo,\n\nSegue em anexo o backup semanal automático do sistema Organize.\n\nData: ${new Date().toLocaleString('pt-BR')}\n\n— Sistema Organize`,
      attachments: [{
        filename: `organize_backup_${ts}.db`,
        path: backupFile
      }]
    });

    console.log('✅ Backup semanal enviado por e-mail!');
  } catch(e) {
    console.error('❌ Erro ao enviar backup:', e.message);
  }
}

// Verificar se é domingo às 8h para enviar backup
function agendarBackupSemanal() {
  setInterval(() => {
    const agora = new Date();
    if (agora.getDay() === 0 && agora.getHours() === 8 && agora.getMinutes() === 0) {
      enviarBackupEmail();
    }
  }, 60 * 1000); // verifica a cada minuto
}

// Rota para backup manual por email (admin)
app.post('/api/backup/email', authAdmin, async (req, res) => {
  await enviarBackupEmail();
  res.json({ ok: true, msg: 'Backup enviado para nildomoraesagro@gmail.com.br' });
});
